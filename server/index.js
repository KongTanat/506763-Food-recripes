...
app.get('/recipes', async (req, res) => {
    ...
    if (token) {
        try {
            ...
            console.warn('Invalid token for fetching recipes, proceeding as guest:', err.message);
...
// --- Rating Routes --- (ส่วนที่เพิ่มเข้ามาใหม่ทั้งหมด)
...
// --- Static File Serving --- (ส่วนที่เพิ่มเข้ามาใหม่)
...
// Catch-all route สำหรับ Single-Page Application (SPA) (ส่วนที่แก้ไข)
// เปลี่ยนจาก app.get('*', ...) เป็น app.use(...) เพื่อความเข้ากันได้ที่ดีขึ้น
app.use((req, res) => {
